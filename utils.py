...
def save_ckpt(epoch, model, optimizer, scheduler, losses, model_name, ckpt_folder):
    ...
    torch.save(
        {
            'epoch': epoch,
            'model_state_dict': getattr(model, 'module', model).state_dict(), # if model has a module attribute (as in DDP or DataParallel), use that; otherwise, just use model directly.
...
def save_best_ckpt2(epoch, model, optimizer, scheduler, losses, acc, f1, output_nodes_names, model_name, ckpt_folder, tag=''):
    """
    save checkpoint
    """
    if not os.path.exists(ckpt_folder):
    ...
    torch.save(
        ...
        f'{ckpt_folder}/{model_name}_best{tag}.pth'
    )
