...
class ConditionEmbedding(nn.Module):
    def __init__(self, dim, num_conditions):
        super().__init__()
        self.emb = nn.Embedding(num_conditions, dim)

    def forward(self, x):
        return self.emb(x)

# Expression embedding with learnable MASK token
class ExpressionEmbedding(nn.Module):
    def __init__(self, output_dim=200, hidden_dims=[16, 64]):
        super().__init__()
        ...
        self.mask_token = nn.Parameter(torch.zeros(output_dim))

    def forward(self, expression_values):
        mask = (expression_values == -1.0)  # Placeholder for masked positions
        embedded = self.mlp(expression_values)
        mask = mask.squeeze(-1)  # shape: (batch_size, num_genes)
        embedded[mask] = self.mask_token  # Replace masked positions
        return embedded
...
class PlantLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        cross_attend = False,
        no_projection = False,
        tie_embed = False,                  # tie token embedding and output projection weights
        gene_emb = False,
        condition_emb = False,
        num_conditions = 0,
        init_token_embed = False,
        auto_check_redraw = True,
        qkv_bias = False,
        mlp_embedding = False,
    ):
        ...
        if self.mlp_embedding:
            self.token_emb = ExpressionEmbedding(output_dim=dim) # embed each gene expression value to a vector of dim
        else:
            self.token_emb = nn.Embedding(num_tokens, dim)
            if init_token_embed:
                self.token_emb.weight.data = torch.tensor([[i / num_tokens for _ in range(dim)] for i in range(num_tokens)])

        if gene_emb:
            self.pos_emb = GeneEmbedding(dim, max_seq_len)
        ...
        if condition_emb:
            assert num_conditions > 0, "num_conditions is 0"
            self.cond_emb = ConditionEmbedding(dim, num_conditions)
        else:
            self.cond_emb = torch.zeros_like
        ...
        self.norm = nn.LayerNorm(dim)
        if self.mlp_embedding:
            self.to_out = nn.Linear(dim, 1) # Predict real-valued expression
        else:
            self.to_out = nn.Linear(dim, num_tokens) if not tie_embed else None

    def embedding_distance_regularization(self):
        # Calculate pairwise distances between consecutive embeddings
        penalty = 0.0
        # exclude first and last embedding vector (which are for special tokens: <CLS>, <MASK>/<PAD>)
        for i in range(1, len(self.token_emb.weight) - 2):
            penalty += torch.norm(self.token_emb.weight[i] - self.token_emb.weight[i + 1], p=2)
        return penalty
    ...
    def forward_with_meta(self, x, meta, return_encodings = False, output_attentions = False, **kwargs):
        b, n, device = *x.shape, x.device
        assert n <= self.max_seq_len, f'sequence length {n} must be less than the max sequence length {self.max_seq_len}'
        ...
        if meta is not None:
           x += torch.broadcast_to(self.cond_emb(meta).unsqueeze(1), x.shape)
